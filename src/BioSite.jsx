...
export default function BioSite() {
  ...
  // Function to get the admin chat list
  const renderChatList = () => {
    try {
      // Get renamed users map from localStorage 
      let renamedUsers = {};
      try {
        const storedRenamedUsers = localStorage.getItem("renamedUsers");
        if (storedRenamedUsers) {
          renamedUsers = JSON.parse(storedRenamedUsers);
        }
      } catch (e) {
      ...
      // First pass: organize messages by participant
      ...
      // Second pass: create JSX elements
      const chatElements = [];
      
      for (const participant in messagesByParticipant) {
        if (!participant || participant === "undefined") continue;
      ...
      return <div className="text-red-500 p-3">Error loading chats. Please refresh the page.</div>;
    }
  };className="text-xs text-red-400 mt-1 hover:text-red-600"
                    onClick={async () => {
                      const confirmDelete = window.confirm("Delete this message?");
